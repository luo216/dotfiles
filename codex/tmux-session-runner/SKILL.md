...
description: Isolated tmux supervisor for long-running and interactive Codex tasks. Use when Codex needs to launch work in a dedicated session, monitor progress, respond to prompts, or interrupt and recover a live shell without attaching.
---

# Tmux Session Runner

Use this skill as Codex's tmux-backed supervisor for any task that is not a short one-shot shell command.
...
- socket path: `/tmp/codex-supervisor-tmux.sock`
- config file: `/dev/null`

That keeps Codex's automation separate from the operator's normal tmux environment.
...
- By default the skill uses `/tmp/codex-supervisor-tmux.sock` and ignores user tmux config via `/dev/null`.
