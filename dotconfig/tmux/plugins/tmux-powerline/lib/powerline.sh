...
format() {
	...
	fg_color_in=$(__normalize_color "182")
	fg_color_re=$(__normalize_color "211")

	case $type in
		inverse)
			echo "fg=$fg_color_re,bg=$bg_color,nobold,noitalics,nounderscore"
			;;
		regular)
			echo "fg=$fg_color_in,bg=$bg_color,nobold,noitalics,nounderscore"
			;;
		*)
			;;
	esac
